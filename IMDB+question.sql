...
FROM   movie; 
#WHERE id or title or year or date_published or duration or country or 
#worldwide_gross_income or languages or production_company IS NULL;
...
-- Type your code below:
SELECT country,
       Sum(total_votes) AS tot_num_votes
FROM   movie AS m
       INNER JOIN ratings AS r
               ON m.id = r.movie_id
...
SELECT *,
       ( CASE
           ...
           WHEN avg_rating >= 7
                AND avg_rating < 8 THEN "Hit"
           WHEN avg_rating >= 5
                AND avg_rating < 7 THEN "One-time watch"
